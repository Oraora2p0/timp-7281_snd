...
int print_width(struct Tree *t)
{
	...
	for (i = 0; i < Rang; i++)//обработка рангов
	{
		...
		while (j != 0)//проход дл€ каждого элемента
		{
			k = 0;
			Temp = t;
			for (k = 0; k < i; k++)//проход до определЄнного узла
			{
				if (comb[k] == 0)
				{
					if ((Temp->Left) != NULL)
			...
			if (i != 0)//измен€ем массив, если он вообще существует
			{
				sk = i - 1;
				comb[sk]++;
				while (1)
